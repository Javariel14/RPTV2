...
async function proxy(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  ...
  const entity = /^[0-9a-f-]{36}(\/commands)?$/i.test(path);
  if (!['ready', 'session', 'context', 'visits'].includes(path) && !entity)
  ...
  try {
    ...
    const response = await fetch(`${origin}/${target}${request.nextUrl.search}`, {
